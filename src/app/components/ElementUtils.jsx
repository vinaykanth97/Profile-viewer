...
export const InputField = (props) => {
    let { labelName, fieldName, type, error, onChange, onBlur, value, ref } = props
    return (
        <div className="mb-7">
            <label className="block text-black font-semibold mb-1">{labelName}</label>
            <input type={type} name={fieldName} className={`border-solid border-zink-200 border-2 w-[100%] p-2 text-black placeholder-zink-100 rounded-[6px] ${error ? 'border-[#d84848]' : ''} `} onChange={onChange} onBlur={onBlur} value={value} ref={ref} />
...
export const ToggleSwitch = (props) => {
    let { title, subtitle, fieldName, value, ref, onChange, onBlur } = props
...
export const PlaygroundCards = ({ techIcon, title, techName, time, cardId }) => {
...
export const ProjectCards = ({ projectImg, title, techName, time, cardId }) => {
        ...
        <label className={`card p-[20px] border-solid  border-2 rounded-[10px] basis-[calc(50%-20px)] flex-grow-1 cursor-pointer relative  ${projectCard ? 'border-[#4F46E5]' : 'border-zink-100'} group hover:bg-primary-600 hover:bg-opacity-10 hover:border-[#4F46E5] group-hover flex-1 bg-zink-50`
            ...
            <div className='mb-6 relative'>
                ...
                <div className="flex flex-wrap absolute bottom-4 left-4 gap-2">
                    <span class="bg-secondary-200 text-blue-800 text-xs font-medium px-2.5 py-0.5 dark:bg-blue-900 text-secondary-900 rounded-[4px]">Badge</span>
                </div>
