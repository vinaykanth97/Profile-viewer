...
export const ProjectCards = ({ projectImg, title, techName, time, cardId, value, onChange, onBlur, selectedItem }) => {
    console.log(selectedItem)
    const [projectCard, setprojectCard] = useState(false)
    return (
        <label className={`card p-[20px] border-solid  border-2 rounded-[10px] basis-[calc(50%-20px)] flex-grow-1 cursor-pointer relative  ${projectCard || selectedItem ? 'border-[#4F46E5]' : 'border-zink-100'} group hover:bg-primary-600 hover:bg-opacity-10 hover:border-[#4F46E5] group-hover flex-1 bg-zink-50`
